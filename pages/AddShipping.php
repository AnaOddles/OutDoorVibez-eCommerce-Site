...
Author:	Ana Sanchez 
Date:   4/3/2019
File:	AddShipping.php
Version: 1.0	 

This file is the script 
that is run when a user 
adds a shipping. It saves the 
shipping that will later
be used in an order.
...
if (isset($_SESSION['userID'])) {
    $shippingID =  $shoppingCart->add_Shipping($_SESSION['userID'], $street, $city, $state, $country,
                        $zipCode, $unitNum, $firstName, $lastName);
    //echo $paymentID;
    $shoppingCart->set_Shipping($_SESSION['orderID'], $shippingID);
    
} else {
    $userID = null;
    $shippingID = $shoppingCart->add_Shipping($userID, $street, $city, $state, $country,
                                         $zipCode, $unitNum, $firstName, $lastName);
    //echo $paymentID;
    $shoppingCart->set_Shipping($_SESSION['orderID'], $shippingID);
}
